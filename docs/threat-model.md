...
- **Archive boundary:** traversal, absolute, drive, UNC, duplicate, case-collision, escaping symlink, special-file, and resource-limit checks reject hostile entry metadata before extraction consumers use it.
...
- **Trust artifacts:** canonical binding checks reject unsigned, unsupported, or mismatched envelopes. No cryptographic signature implementation is claimed.
...
- No cryptographic signing, key management, certificate or revocation model, provenance verification, or transparency-log verification exists yet.
