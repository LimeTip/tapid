...
- The initial macOS design depends on deprecated `sandbox-exec`/Seatbelt behavior. A failed or changed functional probe makes required containment unavailable; it must never fall back to uncontained execution. Linux Landlock plus network controls and Windows AppContainer plus a non-breakaway Job Object are proposed backends only. Neither platform is supported for this contract without positive and negative runtime evidence from the exact integrated commit.
