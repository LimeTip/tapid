...
[ADR 0005](adr/0005-default-on-root-script-sandbox.md) defines two assurance levels for the `tapid run` target architecture. **Restricted** contains requested authority before spawn and propagates it to descendants, while allowing lifecycle cleanup to be best effort when its scope and uncertainty are reported. **ManagedTree** includes Restricted and additionally requires race-free kernel- or VM-owned descendants, a complete cleanup/kill boundary, and configured tree-wide timeout, output, process, and memory semantics. Unsupported required dimensions fail before spawn. Restricted is the planned default for compatible checked-in profiles once a backend exists; ManagedTree selection syntax remains future schema work.

A checked-in `tapid.toml` contains platform-neutral `[run.defaults]` and `[run.scripts.<name>]` profiles with `read`, `write`, `network`, `environment`, `subprocess`, `timeout_seconds`, `max_output_bytes`, `max_processes`, and `max_memory_bytes`. The named script profile overrides only its specified default fields; paths are project-relative, environment entries are names to copy when present rather than values to store, and project configuration cannot disable containment. `network = true` currently grants unrestricted networking. Application `--hostname` and `--port` arguments are not policy, and narrower listen/connect scopes require a future schema.
...
Receipts report every dimension's request, mechanism, assurance level, enforcement state, scope, and limitation. `CanonicalPath` is path-binding evidence with a documented host-race assumption; it is never equivalent to `NativeObject`. Requested, declared, observed, and enforced states remain distinct.

The planned macOS 26 Restricted backend is experimental Seatbelt through deprecated, path-based `sandbox-exec`; it is not implemented or validated, and native ManagedTree remains unsupported. An optional strict Linux VM through Virtualization.framework would be a separate backend with different platform semantics. Linux Restricted is planned around Landlock, `no_new_privs`, seccomp, and runtime capability detection; Linux ManagedTree additionally needs proven namespace ownership and cgroup delegation. Windows is planned around AppContainer or LPAC plus a Job Object, with brokered ports requiring native proof. None of these backends exists at the current exact HEAD. CLI-to-runner wiring still fails closed before spawn and is not native containment evidence.
