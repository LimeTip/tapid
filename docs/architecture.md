...
macOS 26 is unsupported because Seatbelt does not provide complete descendant lifecycle ownership, process groups are escapable, and public process-scanning APIs cannot close the rapid double-fork race. The Endpoint Security descendants client introduced in macOS 27 is a future entitlement-gated investigation, not a supported backend. The proposed Linux Landlock/seccomp-or-network-namespace backend and Windows AppContainer/Job Object backend are designs, not supported-platform claims. CLI-to-runner wiring and Linux/Windows backends remain pending integrated positive and negative runtime evidence; macOS 26 is explicitly unsupported.
