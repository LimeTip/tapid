...
`python3 scripts/check_architecture.py` scans tracked production Rust files. Tests and generated or build trees are excluded. Eight hundred physical lines is a review threshold, not an absolute Rust language rule. A larger cohesive file requires an explicit path and rationale in `docs/architecture-exceptions.txt`. The checker also applies a 100 physical line entrypoint threshold to `crates/tapid-cli/src/main.rs`; a temporary exception must document any existing migration debt.
