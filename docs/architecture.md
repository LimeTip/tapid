...
`python3 scripts/check_architecture.py` scans tracked production Rust files. Tests and top-level generated or build output trees are excluded, while ordinary production modules named `build` or `generated` remain in scope. Eight hundred physical lines is a review threshold, not an absolute Rust language rule. A larger cohesive file requires an explicit path and rationale in `docs/architecture-exceptions.txt`. The checker also applies a 100 physical line entrypoint threshold to `crates/tapid-cli/src/main.rs`; a temporary exception must document any existing migration debt.
