...
The macOS 26 Restricted backend uses experimental Seatbelt through deprecated/private native APIs with path-based grants; native ManagedTree remains unsupported. Deprecated `sandbox-exec` installs a deny-default parameter-bound profile and launches the current executable private helper. A versioned nonce-bound READY/GO handshake gates target exec until the parent registers kqueue `NOTE_EXEC | NOTE_EXIT`; receipt acceptance requires READY, NOTE_EXEC, and CLOEXEC status EOF. Descriptors close at exec and the environment is explicit. Native sampled controls gate support; they do not prove every Seatbelt operation. Checked receipts distinguish exact root directory data, global metadata, explicit runtime devices, and project authority, and disclose CanonicalPath and best-effort lifecycle limitations. The macOS CLI places a private verified Node hard link before local `.bin`, then ordinary runtime tools. Human and JSON receipts share the [versioned CLI contract](../crates/tapid-cli/README.md). Linux Restricted remains planned around Landlock, `no_new_privs`, and seccomp; Windows remains planned around AppContainer or LPAC plus a Job Object. Unsupported platforms and assurance dimensions fail closed before untrusted spawn.
