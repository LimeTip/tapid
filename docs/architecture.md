...
macOS 26 is unsupported because Seatbelt does not provide complete descendant lifecycle ownership, process groups are escapable, and public process-scanning APIs cannot close the rapid double-fork race. The Endpoint Security descendants client introduced in macOS 27 is a future entitlement-gated investigation, not a supported backend. CLI-to-runner wiring is integrated and fails closed before spawn when support is unavailable. The proposed Linux Landlock/seccomp-or-network-namespace backend and Windows AppContainer/Job Object backend remain designs pending integrated native positive and negative runtime evidence; macOS 26 is explicitly unsupported.
