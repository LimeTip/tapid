...
Node.js 22.6.0 or later is required. Run `node --experimental-strip-types tools/check_architecture.ts` locally and in relevant validation. It reports the 800-line recommendation as an advisory and enforces only explicit hard contracts such as the thin CLI entrypoint threshold.
