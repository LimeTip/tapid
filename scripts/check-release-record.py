...
def check(binary):
    ...
    with tempfile.TemporaryDirectory(prefix='tapid-release-contract-') as temporary:
        ...
        env = {'PATH': str(transport) + os.pathsep + os.environ['PATH'],
               ...
               # The record path is an explicit compatibility fixture. Clean
               # production upgrades use signed discovery by default.
               'TAPID_RELEASE_RECORD_URL': DISCOVERY,
...
''')
        ...
        explicit_env.pop('TAPID_RELEASE_RECORD_URL')
        command(['sh', str(ROOT / 'scripts/install.sh'), '--version', '1.2.3',
                 '--install-dir', str(directory / 'explicit')], explicit_env, directory)
        ...
        require(requests.count(DISCOVERY) == 6,
                f'installer/updater did not consistently use owned discovery: {requests!r}')
        ...
        require('github.com' not in hosts, 'unexpected GitHub fallback')
