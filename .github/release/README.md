...
The workflow verifies the generated manifest twice with `.github/release/verifier`, a small Rust binary whose dependency is the workspace's `tapid-signatures` crate. It calls `tapid_signatures::release::verify`, so verification uses the exact RFC 8785 JCS bytes and signature context used by production clients, not Python's `json.dumps` output. It also binds the manifest's version, tag, and full commit to the workflow inputs before any publication step. The verifier loads the explicitly trusted production keyring and fails closed on malformed, unsigned, tampered, or mismatched metadata.

The workflow builds four native artifacts itself, uploads one artifact per target, and merges them before signing. It uses macOS Intel, macOS ARM, Linux x86_64, and Windows x86_64 runners. Windows is packaged as `.tar.gz`, because ZIP extraction remains unsupported by the client. The protected `TAPID_RELEASE_ED25519_PRIVATE_KEY` secret is exposed only to the signing step after `stable-release` approval. The `advance-stable` job verifies the signed manifest again and publishes deterministic `stable.json` with ordered HTTPS manifest endpoints. No preview channel or source fallback is introduced.
