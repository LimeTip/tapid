...
The workflow expects a preceding build to upload an artifact bundle named `release-artifacts`. The signing job exposes the protected `TAPID_RELEASE_ED25519_PRIVATE_KEY` secret only to the signing step. The `advance-stable` job uses the required GitHub protected environment `stable-release`; configure required reviewers on that environment. It refuses empty or unsigned manifests and emits deterministic `stable.json` with ordered HTTPS manifest endpoints. No preview channel or source fallback is introduced.
