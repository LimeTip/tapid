...
The command requires checked-in `tapid.toml` and an exact `[run.scripts.<name>]` profile; `[run.defaults]` is merged only into that explicitly selected profile. The target default, once a backend is implemented, is ADR 0005 **Restricted** execution: requested filesystem/network authority, explicit environment/PATH and descriptor hygiene, and descendant propagation must be established before spawn, while lifecycle cleanup may be best effort and reported as such. **ManagedTree** additionally requires race-free descendant ownership, complete cleanup/kill, and configured tree-wide timeout, output, process, and memory semantics. The selector for ManagedTree is future schema work. Unsupported required dimensions fail before the shell starts.
...
The CLI uses `tapid-runner::ExecutionRequest` and checked execution exclusively for normal root scripts. Native backends own live child stdout/stderr streaming; the CLI does not replay captured bytes after completion. A successful future result must print a receipt identifying each dimension's request, mechanism, assurance level, enforcement state, scope, and limitation; `CanonicalPath` is not `NativeObject`, and limit terminations remain distinct. The current runner base has no native platform backend, so execution fails closed as `unsupported-containment` before spawn and issues no receipt. The Windows verbatim command boundary and all native backend behavior still require execution on their target operating systems.
